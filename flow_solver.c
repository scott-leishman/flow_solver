...
const char* SEARCH_RESULT_STRINGS[4] = { "successful",
                                         "unsolvable",
                                         "out of memory",
                                         "in progress" };
...
const char*
color_char(const char* ansi_code, char color_out, char mono_out)
{
...
const char*
unprint_board(const game_info_t* info)
{
  if (g_options.display_color) {
    static char buf[256];
    snprintf(buf, 256, "\033[%zuA\033[%zuD", info->size + 2, info->size + 2);
    return buf;
  } else {
    return "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n";
  }
...
void
delay_seconds(double s)
{
  if (g_options.display_fast) {
    s /= 4.0;
  }
#ifdef _WIN32
...
pos_t
pos_from_coords(pos_t x, pos_t y)
{
...
void
pos_get_coords(pos_t p, int* x, int* y)
{
  *x = p & 0xf;
  *y = (p >> 4) & 0xf;
}
...
int
coords_valid(const game_info_t* info, int x, int y)
{
...
pos_t
offset_pos(const game_info_t* info, int x, int y, int dir)
{
...
pos_t
pos_offset_pos(const game_info_t* info, pos_t pos, int dir)
{
...
int
get_wall_dist(const game_info_t* info, int x, int y)
{
...
int
pos_get_wall_dist(const game_info_t* info, pos_t pos)
{
...
cell_t
cell_create(uint8_t type, uint8_t color, uint8_t dir)
{
...
const char*
color_name_str(const game_info_t* info, int color)
{
...
const char*
color_cell_str(const game_info_t* info, cell_t cell)
{
  ...
  switch (type) {
    ...
    default:
      return color_char(
        l->ansi_code, (type == TYPE_INIT ? 'o' : 'O'), l->display_char);
  }
}
...
int
game_can_move(const game_info_t* info,
...
void
game_print_svg(FILE* fp, const game_info_t* info, const game_state_t* state)
{
  ...
  for (size_t y = 0; y < info->size; ++y) {
    ...
    for (size_t x = 0; x < info->size; ++x) {
      ...
      if (cell) {
        ...
        if (type == TYPE_PATH || (type == TYPE_INIT) ||
  ...
  for (int color = 0; color < info->num_colors; ++color) {
    ...
    pos_t pos = (state->completed & (1 << color)) ? info->goal_pos[color]
                                                  : state->pos[color];
...
void
game_save_svg(const char* filename,
...
void
game_print(const game_info_t* info, const game_state_t* state)
{
...
int
game_num_free_coords(const game_info_t* info,
                     const game_state_t* state,
                     int x,
                     int y)
{
...
int
game_num_free_pos(const game_info_t* info, const game_state_t* state, pos_t pos)
{
...
double
game_make_move(const game_info_t* info,
               game_state_t* state,
               int color,
               int dir,
               int forced)
...
int
game_next_move_color(const game_info_t* info, const game_state_t* state)
{
...
int
color_features_compare(const void* vptr_a, const void* vptr_b)
{
...
void
game_order_colors(game_info_t* info,
                  game_state_t* state,
                  const char* user_order)
{
  ...
  if (g_options.order_random) {
    ...
    mergesort(
      cf, info->num_colors, sizeof(color_features_t), color_features_compare);
...
pos_t
region_find(region_t* regions, pos_t p)
{
...
void
region_unite(region_t* regions, pos_t a, pos_t b)
{
...
size_t
game_build_regions(const game_info_t* info,
                   const game_state_t* state,
...
void
game_regions_add_color(const game_info_t* info,
                       const game_state_t* state,
...
int
game_is_deadend(const game_info_t* info, const game_state_t* state, pos_t pos)
{
...
int
game_check_deadends(const game_info_t* info, const game_state_t* state)
{
...
int
game_regions_stranded(const game_info_t* info,
                      const game_state_t* state,
                      size_t rcount,
                      const uint8_t rmap[MAX_CELLS],
                      ...
                      int max_stranded)
{
  ...
  for (int color = 0; color < info->num_colors; ++color) {
    ...
    game_regions_add_color(
      info, state, rmap, state->pos[color], cflag, cur_rflags);

    // Add color flag to all regions for goal_pos
    game_regions_add_color(
      info, state, rmap, info->goal_pos[color], cflag, goal_rflags);
...
void
game_print_regions(const game_info_t* info,
                   const game_state_t* state,
                   uint8_t rmap[MAX_CELLS])
{
...
int
game_is_forced(const game_info_t* info,
               const game_state_t* state,
               int color,
               pos_t pos)
{
...
int
game_find_forced(const game_info_t* info,
                 const game_state_t* state,
                 int* forced_color,
                 int* forced_dir)
{
...
node_storage_t
node_storage_create(size_t max_nodes)
{
...
tree_node_t*
node_storage_alloc(node_storage_t* storage)
{
...
void
node_storage_unalloc(node_storage_t* storage, const tree_node_t* n)
{
...
void
node_storage_destroy(node_storage_t* storage)
{
  free(storage->start);
}
...
int
node_compare(const tree_node_t* a, const tree_node_t* b)
{
...
void
heapq_enqueue(queue_t* q, tree_node_t* node)
{
...
void
_heapq_repair(queue_t* q, size_t i)
{
...
void
fifo_enqueue(queue_t* q, tree_node_t* n)
{
...
tree_node_t*
node_create(node_storage_t* storage,
            tree_node_t* parent,
            const game_info_t* info,
            const game_state_t* state)
{
...
void
node_update_costs(const game_info_t* info, tree_node_t* n, size_t action_cost)
{
...
void
game_animate_solution(const game_info_t* info, const tree_node_t* node)
{
...
int
game_is_free(const game_info_t* info, const game_state_t* state, int x, int y)
{
...
int
game_check_chokepoint(const game_info_t* info,
                      const game_state_t* state,
                      int color,
                      int dir,
                      int n)
...
int
game_check_bottleneck(const game_info_t* info, const game_state_t* state)
{
...
void
game_diagnostics(const game_info_t* info, const tree_node_t* node)
{
...
tree_node_t*
game_validate_ff(const game_info_t* info,
                 tree_node_t* node,
                 node_storage_t* storage)
{
...
int
game_search(const game_info_t* info,
...
const char*
get_argument(int argc, char** argv, int* i)
{
    ...
    if (match_id >= 0) {
      ...
      if (options[match_id].dst_flag) {
        ...
        *options[match_id].dst_flag = options[match_id].dst_value;
        ...
        if (!endptr || *endptr) {
          fprintf(stderr,
                  "error parsing bottleneck limit %s "
                  "on command line!\n\n",
                  opt);
          exit(1);
        }
...
main(int argc, char** argv)
{
  ...
  size_t num_inputs =
    parse_options(argc, argv, input_files, user_orders, hint_files);
  ...
  for (size_t i = 0; i < num_inputs; ++i) {
    ...
    if (game_read(input_file, &info, &state)) {
      ...
      int result = game_search(
        &info, &state, hint_file ? hint : 0, &elapsed, &nodes, &final_state);
  ...
  if (boards > 1) {
    ...
    if (!g_options.display_quiet) {
      ...
      for (int i = 0; i < 3; ++i) {
        if (total_count[i]) {
          printf(
            "%'d %s searches took a total of %'.3f seconds and %'zu nodes\n",
            total_count[i],
            SEARCH_RESULT_STRINGS[i],
            total_elapsed[i],
            total_nodes[i]);
        }
      }
